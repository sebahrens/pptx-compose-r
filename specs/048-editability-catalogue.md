...
| Feature | OOXML location | Status | Rationale | Preservation risk |
| --- | --- | --- | --- | --- |
| Document metadata | `docProps/core.xml` (`dc:title`/`dc:subject`/`dc:creator`/`cp:keywords`) | deferred (FIRST post-V1 edit op) | Standalone part: no `txBody`, no run model, no rels/content-type coupling. Needs only its own op + schema + match guard + tests. | Low — does not need the run-scoped primitive. |
| App / custom metadata | `docProps/app.xml` (stats), `docProps/custom.xml` | preserve-only | `app.xml` stats go stale after lifecycle ops; regenerate later. | Preserved as blobs. |
